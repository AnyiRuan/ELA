// Save website content from admin
const fs = require('fs');
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // NOTE: In Vercel, /tmp is the only writable directory
    // For production, consider using Vercel KV, Postgres, or another database
    try {
      const storagePath = path.join('/tmp', 'ela-content.json');
      fs.writeFileSync(storagePath, JSON.stringify(contentData, null, 2));
      console.log('Content saved to:', storagePath);
    } catch (err) {
